...
function getAstpIdByNameAndBranch($conn, $name, $branch) {
    $sql = "SELECT astp_id FROM `astp` WHERE name = ? AND branch = ?";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, 'ss', $name, $branch);
...
<head>
    ...
    <style>
        ...
        .content {
            ...
            align-items: center;
            padding: 10px;
            margin-right: 150px; /* Adjust margin-right: 150px; /* Adjust this value to move the dashboard to the left */
        }
